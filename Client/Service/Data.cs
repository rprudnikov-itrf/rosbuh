...
namespace RosService.Data
{
    public partial class DataClient
    {
        ...
        {
            return RosService.Helper.ConvertDataValue(ПолучитьЗначение(id_node, attributes, хранилище, Client.Domain));
        }
        public Dictionary<string, object> ПолучитьЗначенияФормы(decimal id_node, string Шаблон, Хранилище хранилище)
        {
            return RosService.Helper.ConvertDataValue(ПолучитьЗначенияФормы(Шаблон, id_node, хранилище, Client.Domain));
        }
        public Dictionary<string, object> ПолучитьЗначенияФормы(decimal id_node, Хранилище хранилище)
        {
            return RosService.Helper.ConvertDataValue(ПолучитьЗначенияФормы(null, id_node, хранилище, Client.Domain));
        }
        ...
        public T ПолучитьЗначение<T>(object id_node, string attribute, Хранилище хранилище)
        {
            try
            {
                ...
                {
                    var value = ПолучитьЗначениеПростое(id_node, attribute, хранилище, Client.Domain);
                    if (value is T)
                    {
                        return (T)value;
                    }
                ...
                else
                {
                    ...
                    else if (value.Значение == null && typeof(T) == typeof(DateTime))
                    {
                        return (T)(object)DateTime.MinValue;
                    }
                    else
        ...
        public void СохранитьЗначение(object id_node, string attribute, object value, bool ДобавитьВИсторию)
        {
            ...
            if (value is string
                ...
                || value is byte[])
            {
                СохранитьЗначениеПростое(id_node, attribute, value, ДобавитьВИсторию, Хранилище.Оперативное, Client.UserName, Client.Domain);
            }
            else
            {
        ...
        public void СохранитьЗначение(object id_node, string attribute, object value, Хранилище хранилище)
        {
            if (value is string
                ...
                || value is byte[])
            {
                СохранитьЗначениеПростое(id_node, attribute, value, true, хранилище, Client.UserName, Client.Domain);
            }
            else
            {
                var values = new Dictionary<string, object>();
        ...
        {
            var values = RosService.Helper.ConvertDataValue(значения);
        ...
        #region Кеш
        public T ПолучитьКешЗначение<T>(string key)
        {
            try
            {
                var value = ПолучитьКешЗначение(key, Client.Domain);
                if (value is T)
                {
                    return (T)value;
                }
        ...
        public void СохранитьКешЗначение(string key, object value)
        {
            СохранитьКешЗначение(key, value, 0, Client.Domain);
        }
        public void СохранитьКешЗначение(string key, object value, int timeout)
        {
            СохранитьКешЗначение(key, value, timeout, Client.Domain);
        }


        public void ОбновитьЗначениеВКеше(string ИмяКеша, decimal id_node, string Аттрибут, object Значение)
        {
            var dic = new Dictionary<string, object>();
            dic.Add(Аттрибут, Значение);
            ОбновитьЗначениеВКеше(ИмяКеша, id_node, dic);
        }
        public void ОбновитьЗначениеВКеше(string ИмяКеша, decimal id_node, Dictionary<string, object> values)
        {
            ОбновитьЗначениеВКеше(ИмяКеша, new decimal[] { id_node }, values);
        }
        public void ОбновитьЗначениеВКеше(string ИмяКеша, IEnumerable<decimal> id_nodes, string Аттрибут, object Значение)
        {
            var dic = new Dictionary<string, object>();
            dic.Add(Аттрибут, Значение);
            ОбновитьЗначениеВКеше(ИмяКеша, id_nodes, dic);
        }
        public void ОбновитьЗначениеВКеше(string ИмяКеша, IEnumerable<decimal> id_nodes, Dictionary<string, object> values)
        {
            ОбновитьЗначениеВКеше(ИмяКеша, id_nodes.ToArray(), values, Client.UserName, Client.Domain);
        }
        #endregion

        #region Асинхронно
        public void СохранитьЗначениеАсинхронно(object id_node, Dictionary<string, object> значения)
        {
            var values = RosService.Helper.ConvertDataValue(значения);
            if (values != null && values.Count() > 0)
            {
                СохранитьЗначениеАсинхронно(id_node, values, true, Хранилище.Оперативное, Client.UserName, Client.Domain);
            }
        }
        public void СохранитьЗначениеАсинхронно(object id_node, Dictionary<string, object> значения, bool ДобавитьВИсторию)
        {
            var values = RosService.Helper.ConvertDataValue(значения);
            if (values != null && values.Count() > 0)
            {
                СохранитьЗначениеАсинхронно(id_node, values, ДобавитьВИсторию, Хранилище.Оперативное, Client.UserName, Client.Domain);
            }
        }
        public void СохранитьЗначениеАсинхронно(object id_node, Dictionary<string, object> значения, Хранилище хранилище)
        {
            var values = RosService.Helper.ConvertDataValue(значения);
            if (values != null && values.Count() > 0)
            {
                СохранитьЗначениеАсинхронно(id_node, values, true, хранилище, Client.UserName, Client.Domain);
            }
        }

        public decimal ДобавитьРазделАсинхронно(object id_parent, string тип, Dictionary<string, object> значения)
        {
            if (Client.ПередДобавлением != null)
                Client.ПередДобавлением(this, new AddClientEventArgs(id_parent, тип, значения));

            var values = RosService.Helper.ConvertDataValue(значения);
            var id_node = ДобавитьРазделАсинхронно(id_parent, тип, values, true, Хранилище.Оперативное, Client.UserName, Client.Domain);
        ...
        public decimal ДобавитьРазделАсинхронно(object id_parent, string тип, Dictionary<string, object> значения, bool ДобавитьВИсторию)
        {
            if (Client.ПередДобавлением != null)
                Client.ПередДобавлением(this, new AddClientEventArgs(id_parent, тип, значения));

            var values = RosService.Helper.ConvertDataValue(значения);
            var id_node = ДобавитьРазделАсинхронно(id_parent, тип, values, ДобавитьВИсторию, Хранилище.Оперативное, Client.UserName, Client.Domain);
        ...
        public decimal ДобавитьРазделАсинхронно(object id_parent, string тип, Dictionary<string, object> значения, Хранилище хранилище, bool ДобавитьВИсторию)
        {
            if (Client.ПередДобавлением != null)
                Client.ПередДобавлением(this, new AddClientEventArgs(id_parent, тип, значения));

            var values = RosService.Helper.ConvertDataValue(значения);
            var id_node = ДобавитьРазделАсинхронно(id_parent, тип, values, ДобавитьВИсторию, хранилище, Client.UserName, Client.Domain);
        ...
        public void УдалитьРазделБезПодструктуры(decimal[] id_node)
        {
            УдалитьРазделБезПодструктуры(true, id_node, Хранилище.Оперативное, Client.UserName, Client.Domain);
        }
        public void УдалитьРазделБезПодструктуры(bool ВКорзину, decimal[] id_node)
        {
            УдалитьРазделБезПодструктуры(ВКорзину, id_node, Хранилище.Оперативное, Client.UserName, Client.Domain);
        }
        public void УдалитьРазделБезПодструктуры(bool ВКорзину, decimal[] id_node, RosService.Data.Хранилище хранилище)
        {
            УдалитьРазделБезПодструктуры(ВКорзину, id_node, хранилище, Client.UserName, Client.Domain);
        }
        ...
        {
            УдалитьРаздел2(false, true, query, хранилище, Client.UserName, Client.Domain);
        }
        ...
        {
            ПереместитьРаздел(id_node, ПереместитьВРаздел, ОбновитьИндексы, хранилище, Client.Domain);
        }


        public ФайлИнформация[] СписокФайлов(object id_node)
        {
            return СписокФайлов((decimal)id_node, Хранилище.Оперативное, RosService.Client.Domain);
        }
        public ФайлИнформация[] СписокФайлов(object id_node, Хранилище хранилище)
        {
            return СписокФайлов((decimal)id_node, хранилище, RosService.Client.Domain);
        }
        ...
        //{
        //    return ДобавитьРаздел(id_parent, value.GetType().Name, RosService.Helper.ConvertDataValue(value.v), true, Хранилище.Оперативное, 
        ...
        //        RosService.Helper.ConvertDataValue(value.v),
    ...
    public partial class TableValue : IDisposable
    {
        private DataTable __Значение;
        [System.Xml.Serialization.XmlIgnore]
    ...
    public partial class Query 
    {
        ...
        public static bool ПроверитьМестоПоиска(object id_node)
        {
            if (id_node == null)
                return false;
            else if (id_node is string && !string.IsNullOrEmpty(Convert.ToString(id_node)))
                return true;
            if (id_node is decimal && Convert.ToDecimal(id_node) > 0)
                return true;
            else
            {
                try
                {
                    if (Convert.ToDecimal(id_node) > 0)
                        return true;
                }
                catch
                {
        ...
        public void ДобавитьМестоПоиска(object id_node, int МаксимальнаяГлубина)
        {
    ...
    //    internal protected DataClient client;
